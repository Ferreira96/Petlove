...
class CadastroActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btCadastro.setOnClickListener {
            ...
            if(!cad_email.equals("") or !cad_senha.equals("")){
                createUserComEmailESenha(cad_email, cad_senha)
            }else{
    ...
    /**AUTENTICAÇÃO*/
    private fun createUserComEmailESenha(email: String, password: String){
        auth.createUserWithEmailAndPassword(email, password).addOnCompleteListener{ task->
            if(task.isSuccessful){
                Log.d("EmailAndPassword","createUserComEmailESenha-Sucesso")
                val user = auth.currentUser
            }else{
                Log.d("EmailAndPassword","createUserComEmailESenha-Falha", task.exception)
